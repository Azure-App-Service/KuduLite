...
namespace Kudu.Services.Infrastructure
{
    public static class VfsSpecialFolders
    {
        ...
        public static bool TryHandleRequest(HttpRequest request, string path, out IActionResult response)
        {
            response = null;
            if (String.Equals(path, SystemDrivePath, StringComparison.OrdinalIgnoreCase))
            {
                ...
                location.Path += "/";
                response = new RedirectResult(location.Uri.ToString(), false, true);
            }
        ...
        public static bool TryParse(RouteData routeData, out string result)
        {
            result = null;
