...
namespace Kudu.Core.Deployment.Oryx
{
    public class ExpressBuilder : ExternalCommandBuilder
    {
        ...
        public string SetupK8Artifacts(DeploymentContext context, IEnvironment environment, string outputPath)
        {
            ...
            string createdZip = PackageArtifactFromFolder(context, context.BuildTempPath + "/publish", environment.RepositoryPath + "/../artifacts/" + environment.CurrId, zipAppName, BuildArtifactType.Squashfs, numBuildArtifacts: -1);
            return zipAppName;
        }
